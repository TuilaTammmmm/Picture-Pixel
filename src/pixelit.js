...
class pixelit {
  constructor(config = {}) {
    ...
    //hide image element
    this.hideFromImg();
    //range between 0 to 100 (input 0..100 -> 0..1.0)
    this.scale =
      config.scale && config.scale > 0 && config.scale <= 100
        ? config.scale * 0.01
        : 8 * 0.01;
  ...
  /**
   *
   * @param {int} scale set pixelate scale [0...100]
   */
  setScale(scale) {
    this.scale = scale > 0 && scale <= 100 ? scale * 0.01 : 8 * 0.01;
    this.pixelSize = null; // Clear pixel size when scale is set
    return this;
  }

  /**
   *
   * @param {int} size set exact pixel block size (e.g., 8, 16)
   */
  setPixelSize(size) {
    this.pixelSize = size > 0 ? size : null;
    return this;
  }
  ...
  pixelate() {
    ...
    let workScale = this.pixelSize ? Math.min(1, this.pixelSize / natW) : this.scale;
    ...
    if (natW > 900 || natH > 900) {
      if (!this.pixelSize) {
        // Only apply this scaling hack if we are using the percentage-based scale
        workScale *= 0.5;
      }
